...
explore: ip_targets_style {
  ...
  join: units_by_month{
    type: left_outer
    sql_on: ${ip_targets_style.style} = ${units_by_month.title} AND CAST(${ip_targets_style.target_time_date} AS DATE) = CAST(${units_by_month.created_timestamp_time_date} AS DATE);;
