view: order_tags_by_order {
  ...
  dimension: is_preorder {
    case: {
      when: {
        sql: CONTAINS_SUBSTR(${TABLE}.order_tag_list_test,'narvar') OR CONTAINS_SUBSTR(${TABLE}.order_tag_list_test,'Pre-Order') OR CONTAINS_SUBSTR(${TABLE}.order_tag_list_test,'pre-order')  ;;
        label: "Yes"
      }
